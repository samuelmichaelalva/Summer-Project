...
const rules: Record<string, string[]> = {
  Students: ["student", "education", "scholar"],
  Farmers: ["farmer", "agriculture", "kisan"],
  Women: ["women", "woman", "female", "maternal"],
  "Senior Citizens": ["senior", "pension", "elderly"],
  "Job Seekers": ["job", "employment", "skill", "unemployed"],
  "Entrepreneurs / MSMEs": ["business", "entrepreneur", "msme", "startup"],
  "Persons with Disabilities (PwD)": ["disab", "pwd"],
  "SC / ST / OBC": ["sc", "st", "obc", "caste"],
  "Economically Weaker Section (EWS) / BPL": ["income", "bpl", "ews", "poor"],
...
export function SchemeBrowser() {
  ...
  useEffect(() => {
    fetch("/api/profile")
      ...
      .then((data) => data?.success && setProfile(data.profile));
  }, []);
    ...
    <>
      <label className="relative mb-8 block max-w-xl">
        ...
        <input
          ...
          className="h-11 w-full rounded-full bg-surface-container pl-10 pr-4 text-sm"
      ...
      {eligible.length > 0 && (
        <section className="mb-10">
          <h2 className="mb-5 text-2xl font-extrabold">
            <T id="schemesEligibleHeading" />
          </h2>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {eligible.map((scheme) => (
              <SchemeLink key={`${scheme.category}-${scheme.slug}`} scheme={scheme} href={href(scheme.slug)} />
            ))}
          </div>
        </section>
      )}
  ...
  return (
    <Link href={href}>
      <Card className="h-full transition hover:-translate-y-0.5 hover:border-primary">
        ...
        <h3 className="mt-3 font-bold">{scheme.title}</h3>
