...
export function AssistantChat() {
  ...
  const [messages, setMessages] = useState<Array<{ role: "assistant" | "user"; text: string }>>([
    { role: "assistant", text: t("askAbout") },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      setMessages([...next, { role: "assistant", text: data.answer || t("couldNotPrepareAnswer") }]);
    } catch {
      setMessages([...next, { role: "assistant", text: t("aiUnavailable") }]);
    }
  }
    ...
    <Card>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div className="flex-1">
          <h2 className="font-bold">JanSeva <T id="assistant" /></h2>
          <p className="text-xs font-semibold text-secondary"><T id="readyToHelp" /></p>
        </div>
        <button
          ...
          type="button"
        >
          <RefreshCw size={14} /> <T id="refreshChat" />
        </button>
      </div>

      <div className="space-y-4">
        {messages.map((message, index) => (
          <Bubble key={index} mine={message.role === "user"}>
            {message.text}
          </Bubble>
        ))}
      </div>
      ...
      <div className="mt-6 flex gap-2 border-t border-outline-variant pt-4">
        <input
          ...
          className="h-11 flex-1 rounded-full border border-outline-variant bg-white px-4 text-sm"
  ...
  return (
    <div
      className={`max-w-[85%] rounded-2xl p-4 text-sm leading-6 ${
        mine ? "ml-auto rounded-tr-sm bg-primary text-white" : "rounded-tl-sm bg-surface-container-high text-on-surface"
      }`}
    >
      {children}
    </div>
